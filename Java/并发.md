...
## 使用线程池经验
通过一个线程池检查在线测评机，如果测评机掉线了，就把它从map中删掉
![img](../img/线程池使用1.png)

在实习中，其中一个需求是获取全部的团队消息列表，要遍历每个团队id获得对应的消息通知，所以想着新建一个6个线程的线程池(因为每个用户最多只有6个团队空间)，但后来发现AsyncRestTemplate的原理是一样的，所以就直接调用这个方法了。
