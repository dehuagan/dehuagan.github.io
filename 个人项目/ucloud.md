...
有一个每日集成环境，会在上面跑测试套，运行一段时间后，发现接口出现不同程度的恶化，（要求查询接口不超过1s，写接口不超过3s），其中有一个dhcp相关的查询接口时延高达4-5s，需要优化
...
在压测过程中，发现有一些接口出现概率性失败，查看日志发现是sql执行错误，导致接口报500

单独执行这些sql语句，都是简单的update、set操作，没有问题

查看druid源码发现，在获取数据库连接时，会调用validateConnection，针对不同数据库驱动有不同的实现，主要逻辑就是会检查连接是否正常（通过select 1，而不是执行实际要执行的sql），对于PG驱动，有一个默认的超时时间1s（其他数据库驱动没有，能运行正常），当数据库负载比较高时，有可能会出现检查连接时间超过1s，导致报错

解决办法：在配置datasource时，手动设置超时时间
...
### 为什么能比neutron-server提升

原生neutron-server主要痛点：

- 受限于python特性（python解释性语言，java编译型语言），并发度较差，吞吐量难以提升
- 通过ORM操作数据（把 SQL 的大部分操作用对象方法和属性来代替），虽然便于代码编写，但sql语句生成不灵活，产生大量无用关联查询，数据查询低效、api耗时长，增加死锁风险

quark-vpc的优势：

- 根据url请求按需查询字段
- 高并发、高性能、异步编程框架（SpringBoot+akka-actor）
- 数据库读写分离